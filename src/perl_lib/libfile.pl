...
sub GetNumColumns
{
   ...
   my $rows = `head -1 $file_name | transpose.pl | wc -l`;
...
sub ExtractFileLinesBetweenRegexps
{
  ...
  if ( $regexp_from eq $regexp_to ) {
    die "(libfile.pl) ExtractFileLinesBetweenRegexps() - 'from' and 'to' regexps are identical. Give me a break, use grep.\n";
  }
...
sub ExtractAllFileLinesNotBetweenRegexps
{
  ...
  if ( $regexp_from eq $regexp_to ) {
    die "(libfile.pl) ExtractAllFileLinesNotBetweenRegexps() - 'from' and 'to' regexps are identical. Give me a break, use grep.\n";
  }
